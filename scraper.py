            ...
            items = page.evaluate("""() => {
                ...
                document.querySelectorAll('a[href*="page=Benefit"]').forEach(a => {
                    ...
                    if (title && title.length > 3 && title.length < 120) {
                        if (!title.includes('קטגוריות')) {
                            results.push({title, image: img, link: a.href});
                        }
                    }
                });
                return results;
...
def build_html(deals, last_checked, url):
    ...
    if deals:
        for d in deals:
            img_html = f'<img src="{d["image"]}" alt="deal">' if d.get('image') else ''
            cards += f'<div class="deal-card">{img_html}<div class="deal-title">{d["title"]}</div><a href="{d["link"]}" class="btn" target="_blank">לפרטים והטבה</a></div>\n'
    else:
        cards = '<p style="text-align:center;color:#666;">לא נמצאו מבצעים כרגע</p>'
...
.deal-card img{{max-width:100%;border-radius:6px;margin-bottom:10px;display:block}}
.deal-title{{font-weight:bold;margin-bottom:10px}}
.btn{{display:block;background:#000;color:#fff;padding:10px;border-radius:6px;text-align:center;text-decoration:none;font-weight:bold}}
</style>
</head>
<body>
<div class="container">
<h1>🎁 Corporate SUNDAY</h1>
<div class="meta">עדכון אחרון: {last_checked} | {len(deals)} הטבות</div>
{cards}
...
if __name__ == '__main__':
    ...
    html_content = build_html(deals, last_checked, URL)

    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html_content)

    print("Done - deal.json & index.html updated")            });
            return results;
        }""")
